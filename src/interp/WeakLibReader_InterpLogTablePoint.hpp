...
LinearInterp1DPoint(int i0, double d0, double offset, const double *data,
...
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE double
LinearInterp2DPoint(int i0, int i1, double d0, double d1, double offset,
...
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE double
LinearInterp3DPoint(int i0, int i1, int i2, double d0, double d1, double d2,
  ...
  return math::Pow10(TriLinear(p000, p100, p010, p110, p001, p101, p011, p111,
                               d0, d1, d2)) -
         offset;
}

AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE double
LinearInterp4DPoint(int i0, int i1, int i2, int i3, double d0, double d1,
                    double d2, double d3, double offset, const double *data,
                    const Layout &layout) noexcept {
  ...
  return math::Pow10(TetraLinear(p0000, p1000, p0100, p1100, p0010, p1010,
                                 p0110, p1110, p0001, p1001, p0101, p1101,
                                 p0011, p1011, p0111, p1111, d0, d1, d2, d3)) -
         offset;
}

AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE double
LinearInterp5DPoint(int i0, int i1, int i2, int i3, int i4, double d0,
                    double d1, double d2, double d3, double d4, double offset,
                    const double *data, const Layout &layout) noexcept {
  ...
  return math::Pow10(PentaLinear(p00000, p10000, p01000, p11000, p00100, p10100,
                                 p01100, p11100, p00010, p10010, p01010, p11010,
                                 p00110, p10110, p01110, p11110, p00001, p10001,
                                 p01001, p11001, p00101, p10101, p01101, p11101,
                                 p00011, p10011, p01011, p11011, p00111, p10111,
                                 p01111, p11111, d0, d1, d2, d3, d4)) -
         offset;
}

template <int ND>
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE double
LinearInterpPointDirect(const int indices[ND], const double fractions[ND],
                        double offset, const double *data,
                        const Layout &layout) noexcept {
  ...
  if constexpr (ND == 1) {
    ...
    return LinearInterp2DPoint(indices[0], indices[1], fractions[0],
                               fractions[1], offset, data, layout);
  } else if constexpr (ND == 3) {
    return LinearInterp3DPoint(indices[0], indices[1], indices[2], fractions[0],
                               fractions[1], fractions[2], offset, data,
                               layout);
    ...
    return LinearInterp4DPoint(indices[0], indices[1], indices[2], indices[3],
                               ...
                               fractions[3], offset, data, layout);
  } else if constexpr (ND == 5) {
    return LinearInterp5DPoint(indices[0], indices[1], indices[2], indices[3],
                               indices[4], fractions[0], fractions[1],
                               fractions[2], fractions[3], fractions[4], offset,
                               data, layout);
  }
}
