...
- This is a header-only library exposing an INTERFACE CMake target.
- GPU kernels are device-qualified with `AMREX_GPU_HOST_DEVICE` for CUDA compatibility.
- Headers include `AMReX_GpuQualifiers.H` and `AMReX_Extension.H`; ensure your include path covers `${AMREX_ROOT}/include`.
- HDF5 tables load into `amrex::TableData<double,4>`; for 5D datasets the final two axes are flattened when allocating storage while preserving row-major layout for interpolation.
- Use `LoadHdf5TableParallel` when running under MPI (rank 0 reads, broadcasts to peers).
