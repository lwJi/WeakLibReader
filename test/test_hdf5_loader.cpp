...
std::once_flag gAmrexInitFlag;

void InitializeAmrexOnce()
{
  std::call_once(gAmrexInitFlag, []() {
    int argc = 0;
    char** argv = nullptr;
    amrex::Initialize(argc, argv);
    std::atexit([]() { amrex::Finalize(); });
  });
}
...
bool CreateReferenceHdf5File(const std::filesystem::path& filePath)
{
  const hsize_t dims[3] = {4, 3, 2};
...
TEST_CASE("HDF5 loader reads table and axes", "[hdf5][loader]")
{
  ...
  REQUIRE(CreateReferenceHdf5File(filePath));
...
TEST_CASE("Parallel loader shares table across ranks", "[hdf5][loader][mpi][loader-parallel]")
{
  InitializeAmrexOnce();
  ...
  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() / "weaklibreader_hdf5_parallel_test.h5";

  int createStatus = 1;
  if (amrex::ParallelDescriptor::MyProc() == root) {
    createStatus = CreateReferenceHdf5File(filePath) ? 1 : 0;
  }
  amrex::ParallelDescriptor::Bcast(&createStatus, 1, root);
  REQUIRE(createStatus == 1);

  WeakLibReader::Hdf5Table table;
  const auto status = WeakLibReader::LoadHdf5TableParallel(
      filePath.string(), table, WeakLibReader::Hdf5LoadConfig{}, root);
  REQUIRE(status == WeakLibReader::Hdf5LoadStatus::Success);
