...
double Encode5dIndexValue(int i0, int i1, int i2, int i3, int i4,
                          double base = 0.0) {
  return base + static_cast<double>(i0) + 10.0 * static_cast<double>(i1) +
         100.0 * static_cast<double>(i2) + 1000.0 * static_cast<double>(i3) +
         10000.0 * static_cast<double>(i4);
}

std::vector<double>
BuildFortranOrdered5dData(const std::array<int, 5> &cOrderDims,
                          double base = 0.0) {
  const std::size_t totalSize = static_cast<std::size_t>(cOrderDims[0]) *
...
void WriteOpacityGridGroup(hid_t file, const std::string &groupName,
                           const std::string &name, const std::string &unit,
                           const std::vector<double> &values, int logInterp) {
  hid_t group =
      H5Gcreate(file, groupName.c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  REQUIRE(group >= 0);
...
void CreateWeakLibEmAbTestFile(const std::filesystem::path &filePath) {
  ...
  hid_t emabGroup = H5Gcreate(file, "EmAb_CorrectedAbsorption", H5P_DEFAULT,
  ...
  const std::array<hsize_t, 4> dims = {
      ...
      static_cast<hsize_t>(nRho), static_cast<hsize_t>(nE)};
  const std::size_t totalSize = static_cast<std::size_t>(nE) * nRho * nT * nYe;
  ...
  for (int ye = 0; ye < nYe; ++ye) {
    for (int temp = 0; temp < nT; ++temp) {
      for (int rho = 0; rho < nRho; ++rho) {
        for (int e = 0; e < nE; ++e) {
          data[idx++] = 1.0 + 10.0 * static_cast<double>(ye) +
...
void CreateWeakLibIsoTestFile(const std::filesystem::path &filePath) {
  ...
  hid_t group = H5Gcreate(file, "Scat_Iso_Kernels", H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT);
  REQUIRE(group >= 0);
  ...
  const std::array<hsize_t, 2> offsetDims = {static_cast<hsize_t>(nMoments),
                                             static_cast<hsize_t>(nOpacities)};
  std::vector<double> offsets(static_cast<std::size_t>(nMoments) * nOpacities,
                              0.0);
  WriteDoubleNdDataset<2>(group, "Offsets", offsetDims, offsets);

  const std::array<int, 5> kernelDimsC = {TestNE, nMoments, TestNRho, TestNT,
                                          TestNYe};
  ...
  WriteDoubleNdDataset<5>(group, "Electron Antineutrino", kernelDims,
                          nuBarData);
...
// Write scattering kernel metadata (nOpacities, nMoments, Units, Offsets) into
// an HDF5 group.
...
// Write a 5D kernel dataset from C-order dimensions (stored Fortran-reversed in
// HDF5).
void WriteKernel5d(hid_t group, const char *name,
                   const std::array<int, 5> &cDims, double base) {
  const std::array<hsize_t, 5> hdfDims = {
      static_cast<hsize_t>(cDims[4]), static_cast<hsize_t>(cDims[3]),
      static_cast<hsize_t>(cDims[2]), static_cast<hsize_t>(cDims[1]),
...
// NES and Pair share the same structure: eta grid, 2 moments, 1 opacity,
// dataset name "Kernels".
void CreateWeakLibEtaScatTestFile(const std::filesystem::path &filePath,
  ...
  hid_t group =
      H5Gcreate(file, groupName, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  REQUIRE(group >= 0);
...
void CreateWeakLibBremTestFile(const std::filesystem::path &filePath) {
  ...
  hid_t group = H5Gcreate(file, "Scat_Brem_Kernels", H5P_DEFAULT, H5P_DEFAULT,
                          H5P_DEFAULT);
  REQUIRE(group >= 0);
  WriteScatKernelMetadata(group, 1, 1, {"1/cm"});
  WriteKernel5d(group, "S_sigma", {TestNE, TestNE, 1, TestNRho, TestNT},
                500000.0);
...
TEST_CASE("LoadWeakLibEmAbTable loads legacy EmAb opacity table",
          "[weaklib][opacity][emab]") {
  ...
  CHECK(emAb.layout.stride[1] ==
        static_cast<std::size_t>(emAb.dimensions[0])); // nRho stride
}

TEST_CASE("LoadWeakLibOpacityTableFull loads EmAb table",
          "[hdf5][weaklib][opacity]") {
  using namespace WeakLibReader;
  AmrexGuard amrex{};

  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_emab_full.h5";
  CreateWeakLibEmAbTestFile(filePath);
...
TEST_CASE("LoadWeakLibOpacityTableFull loads Iso table",
          "[hdf5][weaklib][opacity]") {
  using namespace WeakLibReader;
  constexpr double IsoNuBase = 100000.0;
  constexpr double IsoNuBarBase = 200000.0;
  ...
  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_iso_full.h5";
...
TEST_CASE("LoadWeakLibOpacityTableFull loads NES table",
          "[hdf5][weaklib][opacity]") {
  using namespace WeakLibReader;
  ...
  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_nes_full.h5";
...
TEST_CASE("LoadWeakLibOpacityTableFull loads Pair table",
          "[hdf5][weaklib][opacity]") {
  using namespace WeakLibReader;
  ...
  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_pair_full.h5";
  CreateWeakLibPairTestFile(filePath);

  WeakLibOpacityTable table;
  auto status =
      LoadWeakLibOpacityTableFull(table, "", "", "", filePath.string());
...
TEST_CASE("LoadWeakLibOpacityTableFull loads Brem table",
          "[hdf5][weaklib][opacity]") {
  using namespace WeakLibReader;
  ...
  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_brem_full.h5";
...
TEST_CASE("LoadWeakLibOpacityTableFull loads multiple types",
          "[hdf5][weaklib][opacity]") {
  using namespace WeakLibReader;
  AmrexGuard amrex{};
  const std::filesystem::path emabPath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_emab_multi.h5";
  const std::filesystem::path isoPath = std::filesystem::temp_directory_path() /
                                        "weaklibreader_opacity_iso_multi.h5";
  CreateWeakLibEmAbTestFile(emabPath);
  CreateWeakLibIsoTestFile(isoPath);

  WeakLibOpacityTable table;
  auto status =
      LoadWeakLibOpacityTableFull(table, emabPath.string(), isoPath.string());
...
TEST_CASE("LoadWeakLibOpacityTableFull returns error for missing files",
          "[hdf5][weaklib][opacity]") {
  using namespace WeakLibReader;
  AmrexGuard amrex{};
...
TEST_CASE("LoadWeakLibOpacityTableFull returns error for no files",
          "[hdf5][weaklib][opacity]") {
  using namespace WeakLibReader;
  AmrexGuard amrex{};
  detail::ScopedH5ErrorSuppressor silencer{};
...
TEST_CASE("MakeDeviceCopy creates device copy of opacity table",
          "[hdf5][weaklib][opacity][gpu]") {
  using namespace WeakLibReader;
  AmrexGuard amrex{};
  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_emab_device.h5";
  CreateWeakLibEmAbTestFile(filePath);
  ...
  for (int dim = 0; dim < 3; ++dim) {
    test_helpers::VerifyDeviceRoundTrip(
        deviceTable.thermoState.axisStorage[dim],
...
TEST_CASE("LoadWeakLibOpacityTableFullParallel loads EmAb and Iso",
  ...
  const auto emabPath = std::filesystem::temp_directory_path() /
                        "weaklibreader_opacity_emab_parallel.h5";
  const auto isoPath = std::filesystem::temp_directory_path() /
                       "weaklibreader_opacity_iso_parallel.h5";
  CreateWeakLibEmAbTestFile(emabPath);
  CreateWeakLibIsoTestFile(isoPath);
  ...
  auto seqStatus = LoadWeakLibOpacityTableFull(seqTable, emabPath.string(),
                                               isoPath.string());
  ...
  for (int i = 0; i < 3; ++i) {
    CHECK(parTable.thermoState.axes[i].scale ==
          seqTable.thermoState.axes[i].scale);
  }
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    for (std::size_t j = 0; j < parTable.thermoState.axisStorage[i].size();
         ++j) {
  ...
  const auto isoOffsetSize =
      static_cast<std::size_t>(seqTable.scatIso.nOpacities) *
      seqTable.scatIso.nMoments;
  ...
  const auto isoDataSize =
      static_cast<std::size_t>(seqTable.scatIso.dimensions[0]) *
      seqTable.scatIso.dimensions[1] * seqTable.scatIso.dimensions[2] *
      seqTable.scatIso.dimensions[3] * seqTable.scatIso.dimensions[4];
...
TEST_CASE("MakeDeviceCopy creates device copy of Iso table",
          "[hdf5][weaklib][opacity][gpu]") {
  using namespace WeakLibReader;
  AmrexGuard amrex{};
  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_iso_device.h5";
  CreateWeakLibIsoTestFile(filePath);
...
TEST_CASE("MakeDeviceCopy creates device copy of NES table",
          "[hdf5][weaklib][opacity][gpu]") {
  using namespace WeakLibReader;
  AmrexGuard amrex{};
  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_nes_device.h5";
  CreateWeakLibNESTestFile(filePath);

  WeakLibOpacityTable hostTable;
  auto status =
      LoadWeakLibOpacityTableFull(hostTable, "", "", filePath.string());
  REQUIRE(status == Hdf5LoadStatus::Success);
  std::filesystem::remove(filePath);
...
TEST_CASE("MakeDeviceCopy creates device copy of Pair table",
          "[hdf5][weaklib][opacity][gpu]") {
  using namespace WeakLibReader;
  AmrexGuard amrex{};
  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_pair_device.h5";
  CreateWeakLibPairTestFile(filePath);
...
TEST_CASE("MakeDeviceCopy creates device copy of Brem table",
          "[hdf5][weaklib][opacity][gpu]") {
  using namespace WeakLibReader;
  AmrexGuard amrex{};
  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_brem_device.h5";
  CreateWeakLibBremTestFile(filePath);
...
TEST_CASE("ReleaseDeviceData frees scattering kernel device memory",
          "[hdf5][weaklib][opacity][gpu]") {
  using namespace WeakLibReader;
  AmrexGuard amrex{};
  const std::filesystem::path filePath =
      std::filesystem::temp_directory_path() /
      "weaklibreader_opacity_iso_release.h5";
  CreateWeakLibIsoTestFile(filePath);
