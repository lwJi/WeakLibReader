...
// axes and layout.
std::vector<double> BuildSyntheticTable(WeakLibReader::Axis axes[3],
  ...
  for (int iRho = 0; iRho < nD; ++iRho) {
    for (int iTemp = 0; iTemp < nT; ++iTemp) {
      for (int iYe = 0; iYe < nY; ++iYe) {
        ...
        data[layout.Offset(iRho, iTemp, iYe)] = std::log10(E + Offset);
      }
    }
  }
  return data;
  ...
  for (int iRho = 0; iRho < nD; ++iRho) {
    for (int iTemp = 0; iTemp < nT; ++iTemp) {
      for (int iYe = 0; iYe < nY; ++iYe) {
        ...
        d.energy[off] = std::log10(
            EnergyFunc(gridD[iRho], gridT[iTemp], gridY[iYe]) + Offset);
        d.pressure[off] = std::log10(
            PressureFunc(gridD[iRho], gridT[iTemp], gridY[iYe]) + Offset);
        d.entropy[off] = std::log10(
            EntropyFunc(gridD[iRho], gridT[iTemp], gridY[iYe]) + Offset);
      }
    }
  }

  d.bounds = InitializeEosInversionBounds(d.axes, totalSize, d.energy.data(),
                                          Offset, d.pressure.data(), Offset,
...
TEST_CASE("InverseLogInterp recovers T from known bracket", "[eosinversion]") {
  ...
  const double frac = (std::log10(T_target) - std::log10(T_a)) /
                      (std::log10(T_b) - std::log10(T_a));
  const double X_target =
      (X_a + offset) * std::pow((X_b + offset) / (X_a + offset), frac) - offset;

  const double T_recovered =
      detail::InverseLogInterp(T_a, T_b, X_a, X_b, X_target, offset);

  CHECK(std::abs(T_recovered - T_target) / T_target < 1e-12);

  // Also test at a non-midpoint fraction
  const double T_target2 =
      std::pow(10.0, 0.25 * std::log10(T_a) + 0.75 * std::log10(T_b));
  const double frac2 = 0.75;
  const double X_target2 =
      (X_a + offset) * std::pow((X_b + offset) / (X_a + offset), frac2) -
      offset;

  const double T_recovered2 =
      detail::InverseLogInterp(T_a, T_b, X_a, X_b, X_target2, offset);
...
TEST_CASE("CheckInversionInputError returns correct codes", "[eosinversion]") {
  ...
  // Valid input
  CHECK(CheckInversionInputError(1e8, 1e10, 0.3, bounds, bounds.minE,
                                 bounds.maxE) == EosInversionError::Success);

  // D out of range (below)
  CHECK(CheckInversionInputError(1e5, 1e10, 0.3, bounds, bounds.minE,
                                 bounds.maxE) ==
        EosInversionError::DensityOutOfRange);
  // D out of range (above)
  CHECK(CheckInversionInputError(1e11, 1e10, 0.3, bounds, bounds.minE,
                                 bounds.maxE) ==
        EosInversionError::DensityOutOfRange);

  // X out of range (below)
  CHECK(CheckInversionInputError(1e8, 1.0, 0.3, bounds, bounds.minE,
                                 bounds.maxE) ==
        EosInversionError::VariableOutOfRange);
  // X out of range (above)
  CHECK(CheckInversionInputError(1e8, 1e21, 0.3, bounds, bounds.minE,
                                 bounds.maxE) ==
        EosInversionError::VariableOutOfRange);

  // Y out of range (below)
  CHECK(CheckInversionInputError(1e8, 1e10, 0.05, bounds, bounds.minE,
                                 bounds.maxE) ==
        EosInversionError::ElectronFractionOutOfRange);
  // Y out of range (above)
  CHECK(CheckInversionInputError(1e8, 1e10, 0.6, bounds, bounds.minE,
                                 bounds.maxE) ==
        EosInversionError::ElectronFractionOutOfRange);

  // NaN inputs
  const double nan = std::numeric_limits<double>::quiet_NaN();
  CHECK(CheckInversionInputError(nan, 1e10, 0.3, bounds, bounds.minE,
                                 bounds.maxE) == EosInversionError::NaNInput);
  CHECK(CheckInversionInputError(1e8, nan, 0.3, bounds, bounds.minE,
                                 bounds.maxE) == EosInversionError::NaNInput);
  CHECK(CheckInversionInputError(1e8, 1e10, nan, bounds, bounds.minE,
                                 bounds.maxE) == EosInversionError::NaNInput);
}

TEST_CASE("EvalAtFixedTIndex matches full 3D interp at grid T",
          "[eosinversion]") {
  using namespace WeakLibReader;
  ...
  for (double D : testD) {
    for (double Y : testY) {
      ...
      for (int iTemp = 0; iTemp < nT; ++iTemp) {
        ...
        CHECK(std::abs(valFixed - valFull) / (std::abs(valFull) + 1e-30) <
              1e-12);
      }
    }
  }
...
TEST_CASE("Round-trip inversion recovers T without guess", "[eosinversion]") {
  ...
  const auto bounds = InitializeEosInversionBounds(
      axes, totalSize, data.data(), Offset, nullptr, 0.0, nullptr, 0.0);
    ...
    for (double T_known : testT) {
      for (double Y : testY) {
        ...
        const auto error = ComputeTemperatureFromEnergy(
            D, E, Y, axes, data.data(), layout, Offset, bounds, T_recovered);
...
TEST_CASE("Round-trip inversion recovers T with good guess", "[eosinversion]") {
  ...
  double T_recovered = 0.0;
  const auto error = ComputeTemperatureFromEnergy(
      D, E, Y, axes, data.data(), layout, Offset, bounds, tGuess, T_recovered);
...
TEST_CASE("Round-trip inversion recovers T with upper boundary guess",
          "[eosinversion]") {
  using namespace WeakLibReader;
...
TEST_CASE("Inversion at grid boundary points", "[eosinversion]") {
  ...
  for (const auto &pt : points) {
    ...
    double T_recovered = 0.0;
    const auto error = ComputeTemperatureFromEnergy(
        pt.D, E, pt.Y, axes, data.data(), layout, Offset, bounds, T_recovered);
...
TEST_CASE("InitializeEosInversionBounds computes correct bounds",
          "[eosinversion]") {
  using namespace WeakLibReader;
  ...
  CHECK(std::abs(bounds.minE - expectedMinE) /
            (std::abs(expectedMinE) + 1e-30) <
        1e-12);
  CHECK(std::abs(bounds.maxE - expectedMaxE) /
            (std::abs(expectedMaxE) + 1e-30) <
        1e-12);
...
TEST_CASE("InitializeEosInversionBounds populates pressure and entropy bounds",
  ...
  CHECK(std::abs(d.bounds.minP - expectedMinP) /
            (std::abs(expectedMinP) + 1e-30) <
        1e-12);
  CHECK(std::abs(d.bounds.maxP - expectedMaxP) /
            (std::abs(expectedMaxP) + 1e-30) <
        1e-12);
  ...
  CHECK(std::abs(d.bounds.minS - expectedMinS) /
            (std::abs(expectedMinS) + 1e-30) <
        1e-12);
  CHECK(std::abs(d.bounds.maxS - expectedMaxS) /
            (std::abs(expectedMaxS) + 1e-30) <
        1e-12);
}
    ...
    for (double T_known : testT) {
      for (double Y : testY) {
        ...
        const auto error = ComputeTemperatureFromPressure(
            D, P, Y, d.axes, d.pressure.data(), d.layout, Offset, d.bounds,
            T_recovered);
...
TEST_CASE("Round-trip pressure inversion with good guess",
  ...
  double T_recovered = 0.0;
  const auto error = ComputeTemperatureFromPressure(
      D, P, Y, d.axes, d.pressure.data(), d.layout, Offset, d.bounds, tGuess,
      T_recovered);
    ...
    for (double T_known : testT) {
      for (double Y : testY) {
        ...
        const auto error = ComputeTemperatureFromEntropy(
            D, S, Y, d.axes, d.entropy.data(), d.layout, Offset, d.bounds,
            T_recovered);
...
TEST_CASE("Round-trip entropy inversion with good guess",
  ...
  double T_recovered = 0.0;
  const auto error =
      ComputeTemperatureFromEntropy(D, S, Y, d.axes, d.entropy.data(), d.layout,
                                    Offset, d.bounds, tGuess, T_recovered);
