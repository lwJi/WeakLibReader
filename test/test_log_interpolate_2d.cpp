...
TEST_CASE("2D log interpolation matches bilinear expectation",
          "[loginterp][2d]") {
  ...
  Axis axes[2] = {MakeAxis(gridX.data(), 2, AxisScale::Linear),
                  MakeAxis(gridY.data(), 2, AxisScale::Linear)};

  const double x = 1.5;
  const double y = 2.0;
  const double result =
      LogInterpolateSingleVariable2DCustomPoint(x, y, axes, table.data(), 0.0);
...
TEST_CASE("Batch 2D log interpolation matches point wrapper",
          "[loginterp][2d][batch]") {
  using namespace WeakLibReader;
  ...
  const int rc = LogInterpolateSingleVariable2DCustom(
      x0.data(), x1.data(), x0.size(), axes, table.data(), 0.0, out.data());
  REQUIRE(rc == 0);
...
TEST_CASE("Out-of-range on mixed axes extrapolates correctly",
          "[loginterp][policy]") {
  using namespace WeakLibReader;
  ...
  Axis axes[2] = {MakeAxis(gridLog.data(), 2, AxisScale::Log10),
  ...
  const double aLog =
      1.0 / (coords[0] * WeakLibReader::math::Log10(gridLog[1] / gridLog[0]));
  ...
  LinearInterpDeriv2DPoint(idxLog, idxLin, fracLog, fracLin, aLog, aLin, offset,
                           table.data(), layout, expectedInterp, expectedDLog,
                           expectedDLin);

  double interpolant = 0.0;
  double deriv[2] = {0.0, 0.0};
  detail::LogInterpolatedDerivativeDirect<2>(table.data(), layout, axes, coords,
...
TEST_CASE("Template instantiation compiles for all dimensions",
          "[compile-time]") {
  using namespace WeakLibReader;
  ...
  if (false) { // Never executed, but forces template instantiation
    ...
    (void)LinearInterpPointDirect<1>(dummy_indices, dummy_fractions,
                                     dummy_offset, dummy_data, dummy_layout);
    (void)LinearInterpPointDirect<2>(dummy_indices, dummy_fractions,
                                     dummy_offset, dummy_data, dummy_layout);
    (void)LinearInterpPointDirect<3>(dummy_indices, dummy_fractions,
                                     dummy_offset, dummy_data, dummy_layout);
    (void)LinearInterpPointDirect<4>(dummy_indices, dummy_fractions,
                                     dummy_offset, dummy_data, dummy_layout);
    (void)LinearInterpPointDirect<5>(dummy_indices, dummy_fractions,
                                     dummy_offset, dummy_data, dummy_layout);

    // Test LinearInterpDerivPointDirect for 2D-4D (derivatives only)
    double dummy_interp = 0.0;
    double dummy_derivs[4] = {};
    LinearInterpDerivPointDirect<2>(dummy_indices, dummy_fractions,
                                    dummy_scales, dummy_offset, dummy_data,
                                    dummy_layout, dummy_interp, dummy_derivs);
    LinearInterpDerivPointDirect<3>(dummy_indices, dummy_fractions,
                                    dummy_scales, dummy_offset, dummy_data,
                                    dummy_layout, dummy_interp, dummy_derivs);
    LinearInterpDerivPointDirect<4>(dummy_indices, dummy_fractions,
                                    dummy_scales, dummy_offset, dummy_data,
                                    dummy_layout, dummy_interp, dummy_derivs);
  }
