...
class Approx {
public:
  explicit Approx(double value) noexcept : m_value(value) {}
  ...
  friend bool operator==(double lhs, const Approx &rhs) noexcept {
    ...
    const double tol =
        std::max(rhs.m_margin, rhs.m_epsilon * std::fabs(rhs.m_value));
    return diff <= tol;
  }
...
#define TEST_CASE(name, tags)                                                  \
  ...
  static ::simple_catch::TestRegistrar CATCH_INTERNAL_CONCAT(                  \
      test_reg_, __LINE__)(name,                                               \
...
int main() { return ::simple_catch::run_all(); }
#endif
