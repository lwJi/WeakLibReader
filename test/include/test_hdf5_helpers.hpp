...
inline void WriteStringAttribute(hid_t parent, const std::string &name,
                                 const char *value) {
  ...
  hid_t attr =
      H5Acreate(parent, name.c_str(), type, space, H5P_DEFAULT, H5P_DEFAULT);
...
inline void WriteIntArrayDataset(hid_t parent, const std::string &name,
...
inline void WriteDoubleArrayDataset(hid_t parent, const std::string &name,
...
inline void WriteStringArrayDataset(hid_t parent, const std::string &name,
  ...
  hid_t dataset = H5Dcreate(parent, name.c_str(), type, space, H5P_DEFAULT,
                            H5P_DEFAULT, H5P_DEFAULT);
  REQUIRE(dataset >= 0);

  std::vector<char> buffer(values.size() * stride, '\0');
  for (std::size_t i = 0; i < values.size(); ++i) {
    std::memcpy(buffer.data() + i * stride, values[i].c_str(),
                values[i].size());
  }
...
void WriteDoubleNdDataset(hid_t parent, const std::string &name,
...
template <std::size_t N>
void WriteIntNdDataset(hid_t parent, const std::string &name,
                       const std::array<hsize_t, N> &dims,
                       const std::vector<int> &values) {
  hid_t space = H5Screate_simple(static_cast<int>(N), dims.data(), nullptr);
  hid_t dataset = H5Dcreate(parent, name.c_str(), H5T_NATIVE_INT, space,
...
inline void CreateAxisDataset(hid_t file, const std::string &name,
                              const std::vector<double> &values,
                              const char *scale) {
  const hsize_t dims = static_cast<hsize_t>(values.size());
  hid_t space = H5Screate_simple(1, &dims, nullptr);
