...
- **HDF5 Loader:** `WeakLibReader/src/Hdf5Loader.hpp` reads axis metadata + value datasets, validates monotonicity, materializes tables into pinned `amrex::TableData<double,4>`, supports device copies, throttled staggered readers, and includes a rank-0 broadcast helper for MPI setups.
...
* Use `LoadHdf5TableStaggered(..., maxConcurrentReaders)` to limit the number of concurrent file readers when per-rank data loads are required.
